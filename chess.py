...
def move_piece(event):
    global destination_cell, source_cell,turn
    col = event.x // spacing
    row = event.y // spacing
    ...
    if(source_cell is None):
        ...
        if valid_move(x,y,row,col):
            ...
            if is_in_check(board[row][col][1]):
                ...
                turn="Black" if board[row][col][1]=="White" else "White"         # undo/ invalidate/ error throw
        source_cell=None
       

def valid_move(src_row,src_col,dst_row,dst_col,checking=False):
    valid = False
    ...
    # King's Movement defination
        ...
        for dir_row,dir_col in king_direction:
            ...
            if(dst_row==new_row and dst_col==new_col):
                    valid= True
        if(board[src_row][src_col][2]==False):# king hasnt moved
                if(board[src_row][src_col-4][0]=="Rook"):# Rook is its end place
                    if(board[src_row][src_col-4][2]==False):#Rook hasnt moved
                        if(board[src_row][src_col-3] is None and board[src_row][src_col-2] is None and board[src_row][src_col-1] is None):#Check if thereis piecesin between the king and rook
                            if dst_row==src_row and dst_col==src_col-2:# player is castling
                                board[src_row][src_col-1] = board[src_row][src_col-4]
                                board[src_row][src_col-4] = None
                                valid=True 
                                 
                if (board[src_row][src_col+3][0]=="Rook"):# Rook is its end place
                    if(board[src_row][src_col+3][2]==False):#Rook hasnt moved 
                        if(board[src_row][src_col+1] is None and board[src_row][src_col+2] is None):#Check if thereis piecesin between the king and rook  
                            if dst_row==src_row and dst_col==src_col+2:# player is castling  
                                print("kingside castling triggered")
